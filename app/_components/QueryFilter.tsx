...
const QueryFilter = () => {
  ...
  return (<div className="w-full relative top-0  items-center justify-center group rounded flex-col gap-5 hidden sm:flex  transition-all duration-500">
    <form action="" onSubmit={(e) => e.preventDefault()} className="relative flex z-10 justify-end items-end flex-col gap-1 sm:flex-row sm:gap-4">
    ...
    <div className="flex flex-row justify-center text-sm text-white font-bold   ">
      <Link href='/cabins'> <span className='relative bg-black px-1 border-2 text-white hover:cursor-pointer hover:text-[black] hover:bg-white hover:border-black 
         transition-all duration-500'>Show All</span></Link>
    </div>
  </div>
  )
}
